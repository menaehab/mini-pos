...
class UpdateSaleReturnRequest extends FormRequest
{
    ...
    {
        return auth()->user()->can('edit_sale_returns') || auth()->user()->can('manage_sale_returns');
    }
    ...
    public function after(): array
    {
        return [
            function (Validator $validator): void {
                ...
                $currentSaleReturn = $this->route('saleReturn');
                $currentSaleReturnId = $currentSaleReturn instanceof SaleReturn ? $currentSaleReturn->id : null;
                ...
                $returnedQuery = SaleReturnItem::whereIn('product_id', array_keys($requested))
                    ->whereHas('saleReturn', function ($q) use ($sale, $currentSaleReturnId) {
                        $q->where('sale_id', $sale->id);

                        if ($currentSaleReturnId) {
                            $q->where('id', '!=', $currentSaleReturnId);
                        }
                    });
