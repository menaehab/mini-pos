...
export default function Show({ customer }) {
    ...
    const tabs = [
        { id: 'المبيعات', label: 'المبيعات (0)' },
        { id: 'الدفعات', label: 'الدفعات (0)' },
        { id: 'مرتجع', label: 'مرتجع المبيعات (0)' },
    ];
    ...
    return (
        <>
            <Head title={`بيانات ${__('keywords.customer')} - ${customer?.name}`} />
            ...
            <div className="max-w-7xl mx-auto relative font-['Cairo']" dir="rtl">
                ...
                <div className="flex items-center justify-start gap-2 mb-8 text-right">
                    <Link href={route('customers.index')} className="text-gray-500 hover:text-black font-bold text-lg transition-colors">
                        {__('keywords.customers')}
                    </Link>
                    <ChevronLeft size={20} className="text-gray-400 mt-1" />
                    <h1 className="text-xl font-bold text-gray-900">بيانات {__('keywords.customer')}</h1>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                    ...
                    <div className="lg:col-span-1 flex flex-col gap-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                            <h3 className="text-md font-bold text-gray-900 mb-6">{__('keywords.customer')}</h3>
                            ...
                            <div className="flex flex-col gap-4 text-sm font-semibold">
                                ...
                                <div className="flex justify-between items-center">
                                    {/* لم تكن موجودة في ملف الترجمة فكتبتها مباشرة، يمكنك إضافتها لاحقاً */}
                                    <span className="text-gray-500">رقم الهاتف</span> 
                                    <span className="text-gray-900">{customer?.phone || '—'}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-500">تاريخ الإنشاء</span>
                                    <span className="text-gray-900" dir="ltr">{formatDate(customer?.created_at)}</span>
                                </div>
                            </div>
                        </div>

                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                            <h3 className="text-md font-bold text-gray-900 mb-6">رصيد {__('keywords.customer')}</h3>
                    ...
                    <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 min-h-[450px] flex flex-col">
                        ...
                        <div className="flex-1 flex items-center justify-center">
                            <div className="text-center text-gray-400 font-medium text-sm">
                                {activeTab === 'المبيعات' && 'لم يتم العثور على فواتير مبيعات'}
