...
export default function Create() {
    ...
    // داتا وهمية للتجربة (المفروض دي هتيجي من الـ API لما تبحث برقم الفاتورة)
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        reason: '',
        cash_refund: true,
        // items هتشيل الـ id والكمية المرتجعة وحالة التحديد (مختاره ولا لأ)
        items: invoiceProducts.map(p => ({ ...p, return_qty: 0, selected: false }))
    });

    // دالة لحساب عدد المنتجات المحددة وإجمالي المبلغ
    ...
    const handleQtyChange = (id, newQty) => {
        setData('items', data.items.map(item => {
            if (item.id === id) {
                const validQty = Math.min(Math.max(0, newQty), item.max_qty); // عشان ميكتبش رقم سالب أو أكبر من المتاح
    ...
    // تحديث التحديد (الـ Checkbox)
    const toggleSelect = (id) => {
        setData('items', data.items.map(item => {
            if (item.id === id) {
                const newSelected = !item.selected;
                // لو حدده يخلي الكمية 1، لو لغاه يخلي الكمية 0
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        // تصفية المنتجات اللي مش متحددة أو كميتها صفر قبل الإرسال للباك إند
        const payload = {
            ...data,
            items: data.items.filter(item => item.selected && item.return_qty > 0)
        };
        console.log("Data to send:", payload);
        // post(route('sales-returns.store'), payload);
    };
        ...
        <>
            ...
            <div className="relative mx-auto mb-8 max-w-5xl font-['Cairo']" dir="rtl">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                    ...
                    {/* 3. تفاصيل المرتجع (السبب + الاسترداد النقدي + الفوتر) */}
